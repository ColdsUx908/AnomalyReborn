...
{
    private sealed class PublicizerSourceTypeSymbolInfo : TypeSymbolInfoBase
    {
        ...
        public PublicizerSourceTypeSymbolInfo(INamedTypeSymbol symbol, Compilation compilation) : base(symbol)
        {
            if (!Valid)
                return;
    ...
    {
        IncrementalValueProvider<ImmutableArray<PublicizerSourceTypeSymbolInfo>> c = SourceGeneratorHelper.GetCollectedTypeInfos(context, (s, complition) => new PublicizerSourceTypeSymbolInfo(s, complition));
        context.RegisterSourceOutput(c, RegisterAction);
    }

    private static void RegisterAction(SourceProductionContext spc, ImmutableArray<PublicizerSourceTypeSymbolInfo> source)
    {
        ...
        {
            PublicizerSourceTypeSymbolInfo typeInfo = source[i];
            if (i > 0)
                builder.AppendLine();
    ...
    private static string RegisterType(PublicizerSourceTypeSymbolInfo typeInfo)
    {
        StringBuilder builder = new();
    ...
    private static string HandleInstanceFields(PublicizerSourceTypeSymbolInfo typeInfo)
    {
    ...
    private static string HandleInstanceProperties(PublicizerSourceTypeSymbolInfo typeInfo)
    {
    ...
    private static string HandleInstanceMethods(PublicizerSourceTypeSymbolInfo typeInfo)
    {
        ...
        foreach (IMethodSymbol methodSymbol in typeInfo.InstanceMethodSymbols)
        {
            ...
            else
            {
                ...
                {
                    string typeString = $"typeof({p.Type.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat)})";
    ...
    private static string HandleStaticFields(PublicizerSourceTypeSymbolInfo typeInfo)
    {
    ...
    private static string HandleStaticProperties(PublicizerSourceTypeSymbolInfo typeInfo)
    {
    ...
    private static string HandleStaticMethods(PublicizerSourceTypeSymbolInfo typeInfo)
    {
