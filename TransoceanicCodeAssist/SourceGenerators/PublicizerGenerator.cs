...
{
    private sealed class PublicizerTargetSymbolInfo : TypeSymbolInfoBase
    {
        ...
        public PublicizerTargetSymbolInfo(INamedTypeSymbol symbol, Compilation compilation) : base(symbol)
        {
            if (!Valid)
                return;
    ...
    public static string GeneratedCodeMarker => SourceGeneratorHelper.GetGeneratedCodeMarker("PublicizerGenerator");

    private const string HelperMethodClass = "global::Transoceanic.Framework.Helpers.PublicizerHandler.";
    ...
    {
        IncrementalValueProvider<ImmutableArray<PublicizerTargetSymbolInfo>> c = SourceGeneratorHelper.GetCollectedTypeInfos(context, (s, complition) => new PublicizerTargetSymbolInfo(s, complition));
        context.RegisterSourceOutput(c, RegisterAction);
    }

    private static void RegisterAction(SourceProductionContext spc, ImmutableArray<PublicizerTargetSymbolInfo> source)
    {
        ...
        {
            PublicizerTargetSymbolInfo typeInfo = source[i];
            if (i > 0)
                builder.AppendLine();
    ...
    private static string RegisterType(PublicizerTargetSymbolInfo typeInfo)
    {
        StringBuilder builder = new();

        string targetTypeName = typeInfo.TargetTypeSymbol.ToDisplayStringBetter();
        builder.AppendLine($$"""
            {{GeneratedCodeMarker}}
    ...
    private static string HandleInstanceFields(PublicizerTargetSymbolInfo typeInfo)
    {
    ...
    private static string HandleInstanceProperties(PublicizerTargetSymbolInfo typeInfo)
    {
    ...
    private static string HandleInstanceMethods(PublicizerTargetSymbolInfo typeInfo)
    {
        ...
        foreach (IMethodSymbol methodSymbol in typeInfo.InstanceMethodSymbols)
        {
            ...
            else
            {
                ...
                string paramTypesArray = string.Join(", ", methodSymbol.Parameters.Select(p =>
                {
                    string typeString = $"typeof({p.Type.ToDisplayStringWithObjectForNonPublic(null, out _)})";
                    string refSuffix = p.RefKind != RefKind.None ? ".MakeByRefType()" : "";
                    return typeString + refSuffix;
                }));
                ...
                string[] parameterNames = [$"({typeInfo.TargetTypeSymbol.ToDisplayStringBetter()})base.Source", .. methodSymbolInfo.ParameterNamesForCall];
                localBuilder.Append($$"""
                    // {{name}}
                    {{GeneratedCodeMarker}}
                    {{SourceGeneratorHelper.NeverBrowsableIdentifier}}
    ...
    private static string HandleStaticFields(PublicizerTargetSymbolInfo typeInfo)
    {
    ...
    private static string HandleStaticProperties(PublicizerTargetSymbolInfo typeInfo)
    {
    ...
    private static string HandleStaticMethods(PublicizerTargetSymbolInfo typeInfo)
    {
